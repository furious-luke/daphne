...
from tidle import IdleMetrics, MetricsSingleton
...
class Server(object):
    ...
    def __init__(
        ...
        self.timer = IdleMetrics(metrics=['daphne.idle'])
        MetricsSingleton(
            logger=metrics,
            metrics=[
                self.timer
    ...
    def backend_reader_sync(self):
        try:
            self._backend_reader_sync()
        except Exception as e:
            print(str(e))

    def _backend_reader_sync(self):
        """
        ...
        if channels:
            ...
            try:
                ...
                if channel:
                    with self.timer.work('daphne.idle'):
                        # Use no delay.
                        delay = 0.00
